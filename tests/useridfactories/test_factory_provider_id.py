from pas.plugins.authomatic.useridfactories import userid

import pytest


class TestFactory:
    factory = userid.ProviderIDUserIDFactory

    @pytest.fixture(autouse=True)
    def setup(self, user_data, plugin, auth_result_factory):
        self.result = auth_result_factory(user_data)
        self.user_id = self.result.user.id
        self.plugin = plugin

    def test_factory(self):
        """Test that the ProviderIDUserIDFactory generates a valid user ID."""
